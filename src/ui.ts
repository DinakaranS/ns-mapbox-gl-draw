...
import type { DrawContext, DrawUI, MapClasses } from './types';

const classTypes = ['mode', 'feature', 'mouse'] as const;

export default function createUI(ctx: DrawContext): DrawUI {
  const buttonElements: Record<string, HTMLButtonElement> = {};
  let activeButton: HTMLButtonElement | null = null;

  let currentMapClasses: MapClasses = { mode: null, feature: null, mouse: null };
  let nextMapClasses: MapClasses = { mode: null, feature: null, mouse: null };

  function clearMapClasses(): void {
    queueMapClasses({ mode: null, feature: null, mouse: null });
    updateMapClasses();
  }

  function queueMapClasses(options: Partial<MapClasses>): void {
    nextMapClasses = { ...nextMapClasses, ...options };
  }
  ...
  function updateMapClasses(): void {
    ...
    currentMapClasses = { ...currentMapClasses, ...nextMapClasses };
  }
  ...
  function addButtons(): HTMLElement {
    ...
    if (controls.combine_features) {
      buttonElements.combine_features = createControlButton('combineFeatures', {
        ...
        title: 'Combine',
        onActivate: () => ctx.events.combineFeatures(),
        onDeactivate: () => {},
      });
    }
    ...
    if (controls.uncombine_features) {
      buttonElements.uncombine_features = createControlButton('uncombineFeatures', {
        ...
        title: 'Uncombine',
        onActivate: () => ctx.events.uncombineFeatures(),
        onDeactivate: () => {},
      });
    }
